...
redistributionRulesSelected: {
    ...
    saveStatus: null
        ...
        this.instructionScreens = [
            ...
            'The location is rich in natural resources and always has enough food for everyone in the group to survive.',
            ...
            'Today, Person A collected 6 units of food, Person B collected 5 units of food, and Person C collected 4 units of food. \n \nOn occasion, any of the people might collect more or less food than they did today.',
...
showTotalFoodEstimateQuestion ()
{
    ...
    this.addQuestionObject(
        this.add.text(
            640,
            220,
            'How many total units of food do you estimate the group collected today?',
            {
                fontSize: '30px',
                color: '#000000',
                align: 'center',
...
showEqualDivisionMobileTask ()
{
    ...
    this.equalDivisionRemaining = fixedFood.total;
    this.equalDivisionMobileNextShown = false;
    ...
    this.addQuestionObject(this.add.text(
        640,
        90,
        'Imagine what will happen if the group divides the food equally. Use the buttons to give food to each person.',
        {
            fontSize: '28px',
            color: '#000000',
            align: 'center',
    ...
    this.equalDivisionMobileText = this.addQuestionObject(this.add.text(
        640,
        180,
        '',
        {
    ...
    this.createMobileFoodAdjuster(300, 360, 'Person A', 'equal');
    this.createMobileFoodAdjuster(640, 360, 'Person B', 'equal');
    this.createMobileFoodAdjuster(980, 360, 'Person C', 'equal');

    this.updateEqualDivisionMobileText();
}

updateEqualDivisionMobileText ()
{
    this.equalDivisionMobileText.setText(
        `Food left to distribute: ${this.equalDivisionRemaining}\n\n` +
        `Person A: ${this.equalDivisionCounts['Person A']}    ` +
        `Person B: ${this.equalDivisionCounts['Person B']}    ` +
        `Person C: ${this.equalDivisionCounts['Person C']}`
    );

    if (this.equalDivisionRemaining === 0 && !this.equalDivisionMobileNextShown)
    {
        this.equalDivisionMobileNextShown = true;

        this.createNextButton(640, 650, 'Next', () => {
...
createMobileFoodAdjuster (x, y, personLabel, taskType)
{
    this.addQuestionObject(this.add.text(x, y - 90, personLabel, {
        fontSize: '30px',
        color: '#000000'
    }).setOrigin(0.5));

    const minusText = this.add.text(x - 55, y, '−', {
        fontSize: '44px',
        color: '#000000'
    }).setOrigin(0.5);

    const minusButton = this.add.rectangle(x - 55, y, 80, 70, 0xdddddd);
    minusButton.setStrokeStyle(3, 0x000000);
    minusButton.setInteractive({ useHandCursor: true });

    const plusText = this.add.text(x + 55, y, '+', {
        fontSize: '44px',
        color: '#000000'
    }).setOrigin(0.5);

    const plusButton = this.add.rectangle(x + 55, y, 80, 70, 0xdddddd);
    plusButton.setStrokeStyle(3, 0x000000);
    plusButton.setInteractive({ useHandCursor: true });

    minusButton.setDepth(1);
    plusButton.setDepth(1);
    minusText.setDepth(2);
    plusText.setDepth(2);

    const minusAction = () => {
        if (taskType === 'equal')
        {
            if (this.equalDivisionCounts[personLabel] > 0)
            {
                this.equalDivisionCounts[personLabel] -= 1;
                this.equalDivisionRemaining += 1;
                this.updateEqualDivisionMobileText();
            }
        }
        else if (taskType === 'partial')
        {
            if (this.partialRedistributionCounts[personLabel] > 0)
            {
                this.partialRedistributionCounts[personLabel] -= 1;
                this.partialRedistributionPool += 1;
                this.updatePartialRedistributionMobileText();
            }
        }
    };
    ...
    const plusAction = () => {
        if (taskType === 'equal')
        {
            if (this.equalDivisionRemaining > 0)
            {
                this.equalDivisionCounts[personLabel] += 1;
                this.equalDivisionRemaining -= 1;
                this.updateEqualDivisionMobileText();
        ...
        {
            if (this.partialRedistributionPool > 0)
            {
                this.partialRedistributionCounts[personLabel] += 1;
                this.partialRedistributionPool -= 1;
                this.updatePartialRedistributionMobileText();
    ...
    minusButton.on('pointerdown', minusAction);
    minusText.setInteractive({ useHandCursor: true });
    minusText.on('pointerdown', minusAction);

    plusButton.on('pointerdown', plusAction);
    plusText.setInteractive({ useHandCursor: true });
    plusText.on('pointerdown', plusAction);

    this.addQuestionObject(minusButton);
    this.addQuestionObject(minusText);
    this.addQuestionObject(plusButton);
    this.addQuestionObject(plusText);
}
...
showPartialRedistributionMobileTask ()
{
    this.clearQuestionScreen();
    ...
    this.partialRedistributionPool = 0;
    this.partialRedistributionMobileNextShown = false;
    ...
    this.partialRedistributionMobileText = this.addQuestionObject(this.add.text(
        640,
        185,
        '',
        {
    ...
    this.createMobileFoodAdjuster(300, 390, 'Person A', 'partial');
    this.createMobileFoodAdjuster(640, 390, 'Person B', 'partial');
    this.createMobileFoodAdjuster(980, 390, 'Person C', 'partial');
...
updatePartialRedistributionMobileText ()
{
    this.partialRedistributionMobileText.setText(
        `Food available to move: ${this.partialRedistributionPool}\n\n` +
        `Person A: ${this.partialRedistributionCounts['Person A']}    ` +
        `Person B: ${this.partialRedistributionCounts['Person B']}    ` +
        `Person C: ${this.partialRedistributionCounts['Person C']}`
    );
}
...
showPartialRedistributionPreferenceQuestion ()
{
    ...
    const answers = Phaser.Utils.Array.Shuffle([
        'The group should transfer food only as needed to make sure the greatest number of people survive.',
        'Each member of the group should keep the pieces of food he collected.'
    ]);
...
showRedistributionRulesQuestion ()
{
    ...
    this.createCheckboxButton(
        640,
        300,
        'Each person keeps the food he collected.',
        'noRedistribution'
    );
    ...
    this.createCheckboxButton(
        640,
        400,
        'The group divides the food equally among all members.',
        'equalRedistribution'
    );
    ...
    this.createCheckboxButton(
        640,
        500,
        'The group transfers food only as needed to make sure the greatest number of people survive.',
...
if (variableName === 'totalFoodEstimate')
{
    addNext(() => {
        ...
        {
            this.showRotatePhoneScreen(this.showEqualDivisionMobileTask);
        }
        else if (this.isMobileDevice())
        {
            this.showEqualDivisionMobileTask();
        }
        else
        {
...
else if (variableName === 'groupDistributionPreference')
{
    addNext(() => {
        if (this.isMobileDevice() && this.isPortraitMode())
        {
            this.showRotatePhoneScreen(this.showPartialRedistributionMobileTask);
        }
        else if (this.isMobileDevice())
        {
            this.showPartialRedistributionMobileTask();
        }
        else
        {
